-- ============================================================================
-- Add doctor_snapshot to orders + backfill + update submit_order to populate it.
--
-- Why: the lab UI couldn't show the doctor's name/phone because RLS on
-- public.users blocks the lab from reading the doctor's user row when joining
-- doctor_profiles → users. Snapshotting at submit time matches the existing
-- pattern (lab_snapshot / service_snapshot / work_location_snapshot /
-- invoice_recipient_snapshot) and works without RLS gymnastics.
--
...
alter table public.orders
  add column if not exists doctor_snapshot jsonb not null default '{}'::jsonb;

-- 2) Backfill existing rows from doctor_profiles → users
update public.orders o
   set doctor_snapshot = jsonb_build_object(
 ...
 where dp.id = o.doctor_id
   and (o.doctor_snapshot = '{}'::jsonb or o.doctor_snapshot is null);

-- 3) submit_order: include doctor_snapshot on insert
